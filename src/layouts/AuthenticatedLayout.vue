...
<template>
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <!-- Sidebar -->
    <aside
      class="fixed inset-y-0 left-0 z-50 w-64 lg:w-[240px] bg-white border-r border-gray-200 transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:top-0 lg:z-auto lg:h-screen lg:sticky flex flex-col"
    ...
    <div v-if="showLogoutConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-[16px] shadow-xl w-full lg:w-[512px] max-w-sm p-6">
        ...
        <div class="flex gap-3">
          <button @click="showLogoutConfirm = false"
            class="flex-1 h-11 text-sm font-semibold rounded-[8px] border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">
            Cancel
          </button>
          <button @click="handleLogout"
            class="flex-1 h-11 bg-[#D92D20] text-white text-sm font-semibold rounded-[8px] hover:bg-[#A5241B] transition-colors">
            Logout
          </button>
        </div>
      </div>
