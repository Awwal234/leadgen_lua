...
const formatSummaryLinks = (text: string) => {
  ...
  let formatted = text.replace(
    /(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/g,
    '<a href="$1" target="_blank" class="text-blue-600 font-bold hover:underline relative z-10 pointer-events-auto cursor-pointer">$1</a>',
  )

  const domainRegex =
    /(?<!https?:\/\/|\w@)([a-zA-Z0-9.-]+\.(?:com|org|net|io|co|ng|us|uk|ca)(?:\/[^\s<]*[^<.,:;"')\]\s])?)/gi
  formatted = formatted.replace(
    domainRegex,
    '<a href="https://$1" target="_blank" class="text-blue-600 font-bold hover:underline relative z-10 pointer-events-auto cursor-pointer">$1</a>',
  )
  return formatted
}
...
const handleNextProgression = () => {
  if (currentPhase.value === 'research') {
    ...
    currentPhase.value = 'decision-maker'
    companyDomain.value = (companyDomain.value || researchResult.value?.website || '')
      .replace(/^https?:\/\/(www\.)?/, '')
      .split('/')[0]
...
<template>
  <main class="bg-[#fff] flex justify-center items-center h-screen">
    ...
    >
        ...
        <div
          ...
          :class="
            isLoading
              ...
              : 'bg-[#0A0A0A] hover:bg-[#1A1A1A] hover:shadow-[0_6px_20px_rgba(0,0,0,0.15)] cursor-pointer active:scale-[0.98]'
          "
        >
      ...
      <!-- resolve research -->
      <div
        v-show="currentPhase === 'research'"
        class="px-[16px] pt-[90px] pb-[80px] h-full overflow-y-auto w-full overscroll-contain"
        style="scrollbar-width: thin"
      >
        <div
          v-if="isResearching"
          class="flex flex-col items-center justify-center h-full space-y-4 w-full pb-8 pt-4"
        >
          <SkeletonCard :count="3" />
          <p class="text-sm text-gray-500 font-semibold animate-pulse mt-4">
            Gathering Intel on {{ company }}...
          </p>
        </div>

        <div v-else-if="researchResult" class="flex flex-col gap-[20px] pb-4">
          <div
            class="flex flex-col gap-1 items-start bg-blue-50/50 p-4 rounded-xl border border-blue-100"
          >
            <h2 class="text-xl font-bold text-gray-900 leading-tight">
              {{ researchResult.rawResearch?.company || company }}
            </h2>
            <a
              v-if="researchResult.website"
              :href="researchResult.website"
              target="_blank"
              class="text-blue-600 text-sm font-medium hover:underline"
              >{{ researchResult.website }}</a
            >
          ...
          <div
            v-if="researchResult?.summary"
            class="bg-gray-50 p-4 rounded-xl border border-gray-100 mt-1"
          >
            <div class="flex items-center gap-2 mb-3">
              <div class="w-[2px] h-4 bg-gray-400 rounded-full"></div>
              <h3 class="text-[11px] font-bold text-gray-800 uppercase tracking-wider">
                Executive Overview
              </h3>
            </div>
            ...
            <div class="relative">
              <p
                class="text-xs text-gray-700 leading-relaxed font-serif whitespace-pre-line transition-all duration-300 pointer-events-auto"
                :class="{ 'line-clamp-4': !showFullSummary }"
                v-html="formatSummaryLinks(researchResult.summary)"
              ></p>
            </div>
          ...
          <div
            v-if="researchResult.recentNews && researchResult.recentNews.length"
          ...
          >
            ...
            <div
              class="flex overflow-x-auto snap-x snap-mandatory gap-3 pb-2 -mx-4 px-4"
              style="scrollbar-width: none"
            >
              <a
                v-for="news in researchResult.recentNews"
                :key="news.url"
                :href="news.url"
                target="_blank"
              ...
              >
                <h4 class="text-xs font-bold text-gray-900 line-clamp-2 leading-tight">
                  {{ news.title }}
                </h4>
                <p class="text-[10px] text-gray-500 line-clamp-3 leading-relaxed">
                  {{ news.snippet }}
                </p>
                <span class="text-[9px] text-gray-400 font-medium uppercase truncate mt-auto">{{
                  news.date || 'Recent'
                }}</span>
              </a>
            </div>
          ...
          <div
            v-if="researchResult.jobPostings && researchResult.jobPostings.length"
            class="flex flex-col gap-3"
          >
            <div class="flex items-center gap-2">
            ...
            <div class="flex flex-col gap-2">
              <div
                v-for="job in researchResult.jobPostings.slice(0, 4)"
                :key="job.title"
              ...
              >
                ...
                <div class="flex flex-wrap gap-1">
                  <span
                    v-if="job.department"
                    class="px-1.5 py-0.5 bg-gray-200 text-gray-700 rounded text-[9px] font-semibold"
                    >{{ job.department }}</span
                  >
                  <span
                    v-for="signal in job.signals"
                    :key="signal"
                    class="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-[9px] font-semibold"
                    >{{ signal }}</span
                  >
                </div>
              </div>
      ...
      <!-- pain-points phase -->
      <div
        v-show="currentPhase === 'pain-points'"
        class="px-[16px] pt-[90px] pb-[80px] h-full overflow-y-auto w-full overscroll-contain"
        style="scrollbar-width: thin"
      >
        <div class="flex flex-col gap-4">
          <div class="flex flex-col gap-2">
            ...
            <input
              v-model="ourProduct"
              type="text"
              placeholder="Your Product Context (e.g. Crypto exchange platform)"
            ...
            <div
              ...
              class="w-full text-center text-black text-[14px] font-bold rounded-xl py-3 px-4 bg-[#0A0A0A] hover:bg-[#1A1A1A] shadow-[0_4px_14px_0_rgba(0,0,0,0.1)] hover:shadow-[0_6px_20px_rgba(0,0,0,0.15)] cursor-pointer transition-all active:scale-[0.98]"
          ...
          <!-- Results -->
          <div
            v-else-if="painPointsResult && painPointsResult.length > 0"
          ...
          >
            ...
            <div
              v-for="pp in painPointsResult"
              :key="pp.id"
              class="bg-white border border-gray-100 rounded-xl p-3 shadow-sm flex flex-col gap-2 relative overflow-hidden"
            >
              <div
                class="absolute w-1 top-0 bottom-0 left-0"
                :class="pp.urgency === 'high' ? 'bg-red-400' : 'bg-yellow-400'"
              ></div>
              ...
              <p class="text-[11px] text-gray-700 leading-relaxed ml-1 font-serif">
                {{ pp.description }}
              </p>
      ...
      <div
        v-if="
          currentPhase === 'pain-points' &&
          painPointsResult &&
          painPointsResult.length > 0 &&
          !isResolvingPainPoints
        "
        class="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 z-10 w-full rounded-b-[24px]"
      >
        <div
      ...
      <div
        v-show="currentPhase === 'decision-maker'"
        class="px-[16px] pt-[90px] pb-[80px] h-full overflow-y-auto w-full overscroll-contain"
        style="scrollbar-width: thin"
      >
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-3">
            ...
            <div
              @click="handleResolveDecisionMaker"
          ...
          <div v-else-if="decisionMakerResult" class="flex flex-col gap-3 mt-4">
            ...
            >
              ...
              <h4 class="text-xl font-black text-green-900 leading-tight relative z-10">
                {{ decisionMakerResult.name || 'Unhidden Contact' }}
                <span
                  v-if="decisionMakerResult.isPrimary"
                  class="bg-green-100 text-green-700 text-[9px] px-1.5 py-0.5 rounded ml-1 align-top relative -top-0.5"
                  >PRIMARY</span
                >
              </h4>
              <p class="text-[13px] font-semibold text-gray-600 relative z-10">
                {{ decisionMakerResult.title }}
              </p>
              ...
              <div class="flex flex-col gap-1 mt-2 relative z-10">
                ...
                >
                  ...
                  <span class="font-medium truncate">{{
                    decisionMakerResult.email || 'Click to reveal in CRM'
                  }}</span>
                </div>
              </div>

              <a
                v-if="decisionMakerResult.linkedInUrl"
                :href="decisionMakerResult.linkedInUrl"
                target="_blank"
      ...
      <div
        v-if="currentPhase === 'decision-maker' && decisionMakerResult && !isResolvingDecisionMaker"
        class="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 z-10 w-full rounded-b-[24px]"
      >
        <div
          @click="handleNextProgression"
          ...
          <div v-else-if="outreachResult" class="flex flex-col gap-4 mt-4">
            ...
            >
              <div class="bg-gray-800 text-white p-3 flex justify-between items-center">
                ...
                <span class="text-[9px] bg-white/20 px-2 py-0.5 rounded-full"
                  >{{ outreachResult.emailWordCount }} words</span
                >
              </div>
              <div class="p-3 border-b border-gray-100">
                <p class="text-[11px] font-bold text-gray-700 break-words">
                  <span class="text-gray-400 font-normal">Subject:</span>
                  {{ outreachResult.emailSubject || outreachResult.email?.subject }}
                </p>
              </div>
              <div class="p-3 bg-gray-50/50">
                <p class="text-[12px] text-gray-800 leading-relaxed whitespace-pre-wrap font-serif">
                  {{ outreachResult.emailBody || outreachResult.email?.body }}
                </p>
              </div>
            </div>
            ...
            <!-- LinkedIn -->
            <div
              class="bg-blue-50 border border-blue-100 rounded-xl shadow-sm overflow-hidden flex flex-col"
            >
              ...
              <div class="p-3">
                <p class="text-[12px] text-gray-800 leading-relaxed font-serif">
                  {{ outreachResult.linkedinBody || outreachResult.linkedin?.body }}
                </p>
              </div>
            </div>
            ...
            <!-- Notes -->
            <div
              v-if="outreachResult.validationNotes"
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                ...
                <h3 class="text-[10px] font-bold text-orange-900 uppercase tracking-widest">
                  AI Strategist Notes
                  <span class="bg-orange-200 text-orange-800 px-1 rounded ml-1"
                    >{{ outreachResult.validationScore }}/10</span
                  >
                </h3>
              </div>
              <p class="text-[10px] text-orange-800 leading-relaxed whitespace-pre-line">
                {{ outreachResult.validationNotes }}
              </p>
            </div>
          </div>
        </div>
      ...
      <div
        v-if="currentPhase === 'outreach' && outreachResult && !isResolvingOutreach"
        class="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 z-10 w-full rounded-b-[24px]"
      >
        <div
