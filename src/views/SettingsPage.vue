...
<template>
  <div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-[#E6E6E6] p-4 sm:p-6">
      <h2 class="text-lg font-semibold text-gray-900 mb-4">Profile</h2>

      <div class="space-y-4">
        <FormField label="Email">
          <input :value="auth.user?.email" type="email" disabled
            class="w-full h-11 sm:h-11 px-3 rounded-lg border border-gray-200 text-sm bg-gray-50 text-gray-500" />
        </FormField>
        <FormField label="Name" :error="errors.name">
          <input v-model="name" @input="onInput('name')" type="text"
            class="w-full h-11 sm:h-11 px-3 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-black/20 focus:border-black"
