...
<template>
  <div class="bg-white rounded-2xl shadow-stripe-card border border-[#e6ebf1] p-8 sm:p-10 text-center">
    <!-- Icon -->
    <div
      class="w-14 h-14 bg-[#f6f9fc] border border-[#e6ebf1] rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm">
    ...
    <p class="text-[14px] text-[#8898AA] mb-8 leading-relaxed">We sent a 6-digit code to your email. Enter it below to
    ...
    <form @submit.prevent="handleVerify" class="space-y-5">
      ...
      <button type="submit" :disabled="auth.loading"
        class="w-full h-[44px] bg-black text-white text-[15px] font-medium rounded-lg hover:bg-gray-900 shadow-stripe-btn hover:-translate-y-[1px] hover:shadow-stripe-btn-hover active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0">
